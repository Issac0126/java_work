...
public class Singleton {
	...
	//2. 생성자를 호출할 수 있는 영역은 같은 클래스 내부 분이므로
	//자신의 클래스 내부에서 스스로의 객체를 단 1개만 생성함.
	private static Singleton s = new Singleton(); //같은 클래스니까 문제없이 호출 완료
	// private로 s에 직접 접근하는 권한을 막기 
	
	//3. 외부에서 이 클래스의 객체를 요구할 시, 2번에서 미리 만들어 놓은
	//단 하나의 객체의 주소값을 공개된 메서드를 통해 리턴한다. 
	public static Singleton getInstance(){ 
		return s; //객체 없는데 어떻게 호출하냐 -> static으로 호출 가능하게 만듬
	}
