	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 # StringBuilder, StringBuffer (자바 5버전부터 사용 가능)
		  ...
		  - String 클래스 단점으로 메모리 과부하 및 속도가 느려지는 현상을
		   개선하기 위해서 StringBuilder가 새롭게 추가되었다.
		   
		  - StringBuilder는 기존 객체를 계속 재활용하면서 실제 객체
		   내부의 값을 변경하게 된다.
		...
		//문자열을 특정 인덱스에 삽입하는메서드 insert(인덱스, 삽입할문자열)
		sb.insert(6, "my");
		System.out.println(sb);
		
		//특정 인덱스 범위의 문자열을 교체하는 메서드: replace(begin, end, 문자열)
		//끝 범위는 미만으로 인식됨. ?이상부터 ?미만까지.
		sb.replace(6, 8, "your");
		System.out.println(sb);
